...
4. Reconciled attendance for archived events: Past events currently freeze attendees’ counts once the event is removed, which protects historical tallies but limits clean-up when attendance was logged incorrectly. We will add an optional reconciliation mode (e.g., delete event 3 --reconcile) that subtracts the event’s contributions from each attendee and confirms the adjustment in the feedback pane. Example after the enhancement: Removed Outreach Fair 2023. Attendance decremented for 4 members: Amy Tan, Ben Chua, Clarice Ong, Devon Lim. This gives organisers a precise way to correct past records while keeping the standard deletion flow unchanged.

5. Flexible event duration capture: The current add event n/NAME d/YYYY-MM-DDTHH:mm v/VENUE command intentionally stores a single start date-time to keep input compact, preserve existing save files, and support clubs where event durations are implicit. We plan to add an optional end-time parameter (e.g., add event … d/2025-08-01T18:00 e/2025-08-01T20 (line 0)) that performs timeline validation, rejects end times earlier than the start, and updates event cards plus reminders to show the full interval. Organisers who prefer the minimal syntax can keep using the start-only command unchanged.

6. Multi-role membership tagging: Today each member carries one primary role for clarity (add member … r/President), which keeps the UI concise and avoids overloading organisers with dense labels. We will evolve the command to accept repeated role prefixes (e.g., add member … r/President r/Tech Lead) and render them as an ordered list on the member card. This enables committees to reflect dual appointments without duplicating records, while the existing single-role usage keeps working out of the box.

7. Expanded find filters: find member currently accepts only n/ (name) or r/ (role), which keeps the command surface familiar but leaves useful fields—phone p/, email e/, and year y/—outside the search flow. Likewise find event handles n/ (name) and a cascading d/ timestamp that insists on year → month → day → time, so venue v/ and standalone temporal checks remain outside the built-in filters. We will introduce additional prefixes (e.g., find member p/9123, find member e/@u.nus.edu, find member y/2, find event v/COM3) and relax the date parsing so organisers can query by an individual fragment such as month (find event m/Nov), calendar day (find event day/15), or time block (find event t/1800). This preserves the existing quick keyword searches while giving larger committees precision tools when the roster or calendar gets crowded.

8. Expanded find filters: find member currently accepts only n/ (name) or r/ (role), which keeps the command surface familiar but leaves useful fields—phone p/, email e/, and year y/—outside the search flow. Likewise find event handles n/ (name) and a cascading d/ timestamp that insists on year → month → day → time, so venue v/ and standalone temporal checks remain outside the built-in filters. We will introduce additional prefixes (e.g., find member p/9123, find member e/@u.nus.edu, find member y/2, find event v/COM3) and relax the date parsing so organisers can query by an individual fragment such as month (find event m/Nov), calendar day (find event day/15), or time block (find event t/1800). This preserves the existing quick keyword searches while giving larger committees precision tools when the roster or calendar gets crowded.
